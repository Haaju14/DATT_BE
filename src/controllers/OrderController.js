...
export const processPayment = async (req, res) => {
  ...
  try {
    ...
    if (paymentMethod === "cod") {
      ...
      const tempItems = awaitipientItems.findAll({
      ...
      const returnUrl = "http://tmdt2.cholimexfood.com.vn/api/vnpay/callback";
...
export const vnpayCallback = async (req, res) => {
  ...
  try {
    ...
    if (result === 1) {
      ...
      await transaction.commit();
      return res.redirect(
        `http://tmdt2.cholimexfood.com.vn/success?orderId=${orderId}&status=success`
      );
    } else {
      await order.update(
      ...
      await transaction.commit();
      return res.redirect(
        `http://tmdt2.cholimexfood.com.vn/failed?orderId=${orderId}&status=failed&message=${encodeURIComponent(
    ...
    await transaction.rollback();
    return res.redirect(
      `http://tmdt2.cholimexfood.com.vn/failed?orderId=${
